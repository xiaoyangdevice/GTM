...
BATCH_SIZE = 32
GRAD_ACCUM = 2
MAX_LEN = 128  # QNLI 是句子对任务，序列长度适中
EPOCHS = 7     # QNLI 数据集较大，减少epoch数
EPOCHS_OURS = 7
LR = 2e-5
LR_OURS = 2e-5
OUTPUT_DIR = Path("./")
...
def load_qnli():
    ...
    train_ds = ds["train"].map(tokenize, batched=True, remove_columns=["question", "sentence", "idx"])
    val_ds = ds["validation"].map(tokenize, batched=True, remove_columns=["question", "sentence", "idx"])
...
def run_single_model(model_name, train_loader, val_loader, 
    ...
    for epoch in range(epochs):
        ...
        if acc > best_acc:
            ...
            best_f1 = f1
            # 保存最佳模型
            save_path = OUTPUT_DIR / f"{model_name}_qnli_best.pt"
...
def run_qnli_experiment(use_pretrained=True, pooling="mean", models=None):
    
    print("\n" + "="*60)
    print("🚀 QNLI Classification Experiment")
    print("="*60)
    ...
    # 加载数据
    train_loader, val_loader = load_qnli()
    ...
    # 保存结果
    result_file = OUTPUT_DIR / "qnli_results.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="QNLI Classification Experiment")
