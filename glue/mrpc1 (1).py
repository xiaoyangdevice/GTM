...
BATCH_SIZE = 16
GRAD_ACCUM = 2
MAX_LEN = 256  # MRPC 是句子对任务，需要更长序列
EPOCHS = 5
EPOCHS_OURS = 8
LR = 2e-5
LR_OURS = 2e-5
...
def load_mrpc():
    ...
    train_ds = ds["train"].map(tokenize, batched=True, remove_columns=["sentence1", "sentence2", "idx"])
    val_ds = ds["validation"].map(tokenize, batched=True, remove_columns=["sentence1", "sentence2", "idx"])
...
def run_mrpc_experiment(use_pretrained=True, pooling="mean", models=None):
    
    print("\n" + "="*60)
    print("🚀 MRPC Classification Experiment (Fixed)")
    print("="*60)
    ...
    # 加载数据
    train_loader, val_loader = load_mrpc()
    ...
    print("\n" + "="*60)
    print("✅ MRPC Results (Fixed)")
    print("="*60)
    print(df.to_string(index=False))
    
    # 保存结果
    result_file = OUTPUT_DIR / "mrpc_results_fixed3.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MRPC Classification Experiment (Fixed)")
