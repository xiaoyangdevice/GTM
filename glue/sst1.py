...
from glue1 import (
    ...
    RetFormer, GatedMLP, RWKV, RetNet,
...
BATCH_SIZE = 16
GRAD_ACCUM = 2
MAX_LEN = 128  # SST-2 平均长度较短
EPOCHS = 5
EPOCHS_OURS = 8  # Ours 模型使用更多 epoch
LR = 2e-5
LR_OURS = 5e-5  # Ours 模型使用稍高学习率
OUTPUT_DIR = Path("./results_sst2")
...
def load_sst2():
    ...
    train_ds = ds["train"].map(tokenize, batched=True, remove_columns=["sentence", "idx"])
    val_ds = ds["validation"].map(tokenize, batched=True, remove_columns=["sentence", "idx"])
...
def run_single_model(model_name, train_loader, val_loader, use_pretrained=True, pooling="mean"):
    ...
    if ModelClass is None:
        ...
        return {"Model": model_name, "Acc(%)": "N/A", "F1(%)": "N/A", "Mem(GB)": "N/A", "FLOPs(G)": "N/A"}
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} | Loss: {loss:.4f} | Acc: {acc:.4f} | F1: {f1:.4f} | Best: {best_acc:.4f}")
...
def run_sst2_experiment(use_pretrained=True, pooling="mean", models=None):
   
    print("\n" + "="*60)
    print("🚀 SST-2 Classification Experiment (Fixed)")
    print("="*60)
    ...
    if models is None:
        models = [
            ...
            # "Ours-Uni",       # GatedTransMamba + mean池化 + 单向Mamba（对照）
            "Alternate",      # 交替混合
            "Transformer",    # 纯Transformer
            "Mamba",          # 纯Mamba（双向）
            # "Mamba-Uni",      # 纯Mamba（单向，对照）
            "RetFormer",      # RetFormer
            "GatedMLP",       # GatedMLP
            "RWKV",           # RWKV（双向）
            "RetNet",         # RetNet（双向）
    ...
    print("\n" + "="*60)
    print("✅ SST-2 Results (Fixed)")
    print("="*60)
    print(df.to_string(index=False))
    
    # 保存结果
    result_file = OUTPUT_DIR / "sst2_results_fixed2.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SST-2 Classification Experiment (Fixed)")
